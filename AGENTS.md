...
│   ├── src/                    # 源代码
│   │   ├── components/         # 公共组件
│   │   ├── composables/        # 组合式函数（技能《4. 状态与复用》：可复用响应式逻辑，useXxx.ts）
│   │   ├── views/              # 页面组件
...
| 技能                             | 适用场景                 |
| -------------------------------- | ------------------------ |
| **skills-doc-writer**            | 技术方案、项目建议书、可研报告、投标技术方案、实施/验收文档编写 |
...
### 📄 方案与文档编写（skills-doc-writer）

写技术方案、项目建议书、可研报告、投标技术方案、实施/验收文档时用这个技能。技能描述里带了触发词，说"写方案"、"编写技术文档"、"把材料整理成方案"就会自动加载；也可以在对话里直接点名 `skills-doc-writer` 强制使用。

技能内部的硬性要求（详见 `.dsh/skills/skills-doc-writer/references/`）：

- **禁止一口气写完整个文件**：必须走「大纲 → 大纲里标引用 → 按照标的引用去看原文分章写正文」三步——先出三级大纲（`##`~`####`）→ 在大纲里用 `<mark>` 黄底标出每条三级标题引用哪份文件的哪个章节的哪些内容 → 写每节前按标记把原文调出来读、读到什么写什么，分章成文；写的过程中觉得大纲不合理就改大纲（同步改标记与已写章节），loop 到连续 2-3 章不必再动大纲
- **不按章节顺序硬写**：先写约束力最强的章节（范围与职责 → 角色与业务 → 数据对象 → 总体架构 → 功能流程接口 → 指标保障验收风险），最后回头提炼目标、原则与概述，避免总括章节写成空话
- **核心图表不能缺**：长文档先定图表清单（外部关系、业务流程图、总体架构、对象谱系、任务与异常流程、存储编目分发、监控与质量控制等），边写边出图；通篇只有文字必然空泛
- **缺参数先问用户，不许填空**：参数未定时先把问题问出去（一次最多 3 个，附"缺它会导致什么"）；问不到就跳过那一节并记入「待确认事项」（含影响章节、责任方、需取得的资料、最迟确认节点）。正文里不许写推测数值、不许写"待定/视情况"，也不许用"能力/流程/参数项/测算方法"把空话重新包装一遍；只允许写有来源的事实，或待确认表里有责任方与确认节点的引路句
- 有参考文档时优先仿写或摘抄原话（20 字以上完整表述加引号并标注来源），术语与句式跟参考材料保持一致
- 禁止正确的废话：任何行业都能套用、删掉不损失信息的句子一律不写
- 缺信息时先查 ragflow 知识库（`mcp__ragflow__*`）与联网检索，仍不确定就问用户或索取必要文档，不编造条件
- 禁用一眼 AI 的词汇与句式，克制等长分段、段末总结、机械排比等 AI 输出习惯
- 引用不得编造；每篇必须带足本项目专有事实（数字、系统名、场地、日期）

两个确定性校验脚本（零依赖 Node，随技能分发）：

```bash
# 1) 大纲阶段：三级标题是否都标了可核对的引用来源（退出码 1 = 有漏标）
node .dsh/skills/skills-doc-writer/scripts/outline-check.mjs docs/技术方案.outline.md
# 2) 正文阶段：文风与送审硬伤扫描（退出码 1 = 有严重项）
node .dsh/skills/skills-doc-writer/scripts/doc-lint.mjs docs/技术方案.md
```

严重项必须清零，提示项逐条人工判断是否成立。
...
13. **必须遵循 `frontend` 技能（强制前置，不可跳过）**：
    ...
    - **规范范围以技能为准**（本文件不重复）：**项目结构与目录职责（`src/` 各目录分工）**、页面拆分与组件提取、文件行数上限、**JS/TS 语法规范（强制 ES6+，退化到 ES5 必须汇报）**、Composition API（`<script setup>`）、响应式与类型、**状态管理（非必要不用 store，仅复杂组件共享变量如全局 `map` / `viewer` 才入 store）**、**复用与拆分优先 composables**、**代码自包含（就地实现，禁止为抽象而抽象、禁止让人跨文件找实现）**、**节制使用 `watch` / `computed`**、**新建 `.vue` 文件按模板结构创建**（骨架 `reference/temp.vue`、风格示例 `reference/style-sample.vue`）、**类型集中放 `src/types/`（`Res<T>` 等响应/业务类型，禁止在组件内就地声明）**、样式作用域、异步错误处理、注释要求、**地图 / 三维实例管理（单例、`onUnmounted` 销毁、禁止深度响应式包裹）**
    - **级别**：技能内所有条款一律按**必须**执行（技能不再设「推荐」档）
    - **冲突处理**：技能与本文件（含 #14–#16 前端铁律）不一致时取**更严格**者；两者结论相反且都很硬时，停下来问，禁止自行挑选
    - **技能不可用**（文件缺失/读取失败）：先说明情况，再按本文件 + 框架通用最佳实践执行，并在交付说明中标注
14. **跨组件共享状态使用 Pinia（非必要不用 store）**：仅复杂组件间共享的变量才进 store（如全局 `map` / `viewer`），禁止滥用全局状态；落地细则见技能《4. 状态与复用》
...
18. **接口规范**：
    ...
    - 统一响应格式：`{ code: number, msg: string, data: any }`（前端类型即技能《5. 类型与接口》的 `Res<T>`）
